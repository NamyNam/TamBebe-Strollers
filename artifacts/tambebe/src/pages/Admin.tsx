      ...
      <div className="flex items-center gap-4 p-4 cursor-pointer select-none" onClick={() => { setOpen(o => !o); if (open) setAdding(false); }}>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {isExtra
            ...
            : onHide && (
                <button
                  onClick={e => { e.stopPropagation(); if (confirm(`"${product.brand} ${product.model}" modelini silmek istediğinizden emin misiniz?`)) onHide(); }}
                  className="p-1.5 rounded-lg hover:bg-red-100 text-muted-foreground hover:text-red-500"
                  title="Modeli gizle">
                  <Trash2 className="w-4 h-4" />
                </button>
      ...
      <div className="flex items-center gap-3 p-4 cursor-pointer select-none" onClick={() => setOpen(o => !o)}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <p className="text-xs text-muted-foreground font-medium">{req.city}{req.district ? ` / ${req.district}` : ""} · {date}</p>
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          ...
          <div className="bg-gray-50 rounded-xl p-3 space-y-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
              ...
              <span className="flex items-center gap-2 text-muted-foreground font-medium sm:col-span-2">
                <MapPin className="w-3.5 h-3.5 shrink-0" />{req.city}{req.district ? ` / ${req.district}` : ""} — {req.address}
              </span>
            </div>
          </div>
          {req.notes && (
...
export default function Admin() {
  ...
  const { products, storeData, addExtraProduct, deleteExtraProduct, hideProduct, resetAll } = useProductStore();
